...
2026-09-26 增补：`npm run eval:process-recovery` 在独立子进程实际写入本地文件后发送 `SIGKILL`。覆盖副作用已完成／文件回执未提交、回执已提交／执行账本未完成两个窗口，以及前一个窗口后被外部修改的情形。[原始记录](evidence/process-recovery-2026-09-26.json) 显示前两种通过已有审批意图或回执对账，重复写入为 0；外部修改时保持未决，交给人工。不是电源故障／fsync、远程副作用或模型语义质量验证。

同日新增 `npm run eval:recovery-windows`：[真实进程记录](evidence/recovery-windows-2026-09-26.json)确认模型响应后／Session 前崩溃会再生成；回复后／Trace 前崩溃安全停止；Outbox 重投没有重复 Job；双进程租约抢占拒绝旧 Worker 提交。事件重放验证 v2／v3 文件格式兼容、旧审批失效、缺正文与未知事件拒绝，不能扩大为完整业务事件迁移。模型为 Fake，新增付费调用为 0。完整边界见[实施记录](evidence/automatic-eval-2026-09-26.md)。

Runtime Adapter 在构建上下文、接受模型响应与启动工具前重新检查已用时间，避免定时器因休眠或同步阻塞延后时接受逾期成功响应。合同测试模拟了时钟跨过截止时间而定时器尚未执行的情况；这没有证明历史 RI-14 的根因，也没有延长／复活已过期的 Worker 租约。已经落入未知窗口的模型请求仍按原有 Intent 规则停止自动重放。

同日补齐模型调用遥测与在线账本的[固定错误分类](../server/runtime/modelFailure.ts)：沿最多 5 层 cause 识别连接失败、超时、取消、HTTP 状态与受限输出，不保存任意错误文本／URL。成功 HTTP 响应读取正文时的连接中断不再被改记为 JSON 格式错误；明确 HTTP 失败则保留实际状态。这些是调用观测类别，不改变下文 RuntimeFailureCode、重试权限或恢复状态机。旧未知请求不回填原因、用量或费用；离线注入确认未知生成不自动重发。验证与局限见[复核边界专项](evidence/review-boundary-2026-09-26.md)。
