...
2026-09-21 已补充 [检索故障降级](../reliability-recovery.md)：重排暂时不可用时返回混合候选，hybrid 的临时 embedding 故障退回关键词；输出、界面和审计显式标记降级，重新检查权限与来源。离线对照证明控制流可用，未证明降级后的真实检索质量。
...
「导入包装资料与目录」沿用 `open-products` / `npm run knowledge:connect-products`：保留原 6 条产品目录，追加 460 条 FDA 官方 PCR 工艺快照和 10 条自编资料入口，合计 17 份文档/476 块（与原 5 篇研究合计 22 份/1,044 块）。FDA 地区为 US，不能使用 HK/unknown 绕过适用性过滤。完整行一块、0 overlap；原 JATS 分块、固定 E5 和 mMARCO 均不变。新增供应商 TDS 全文仍为 0。授权、原始哈希、固定本地对照与限制见 [扩充报告](expansion-2026-09-21.md)。
