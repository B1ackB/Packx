...
async function main() {
	const { values } = parseArgs({ options: { "allow-runtime-change": { type: "boolean" }, plan: { type: "boolean" }, offline: { type: "boolean" }, online: { type: "boolean" }, "prior-report": { type: "string" }, report: { type: "string" }, "usd-limit": { type: "string" }, repeats: { type: "string" }, "carry-reviewed-stop": { type: "string" } } });
	...
	if (!values["allow-runtime-change"]) assert.equal(sourceHashes["server/runtime/agentRuntime.ts"], frozen.runtime.sourceSha256, "shared_runtime_changed");
